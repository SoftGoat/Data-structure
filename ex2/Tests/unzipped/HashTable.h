...
private:
    ...
    const size_t INITIAL_CAPACITY = 101; ///< Initial capacity of the hash table
...
HashTable<ValueType, KeyType, HashFunc>::HashTable(size_t initialCapacity = INITIAL_CAPACITY, HashFunc hashFunc, float loadFactorThreshold)
...
template <typename ValueType, typename KeyType ,typename HashFunc>
size_t HashTable<ValueType, KeyType, HashFunc>::hash(const KeyType& key, size_t i) const {
...
template <typename ValueType, typename KeyType ,typename HashFunc>
void HashTable<ValueType, KeyType, HashFunc>::resize() {
...
template <typename ValueType, typename KeyType ,typename HashFunc>
bool HashTable<ValueType, KeyType, HashFunc>::insert(const KeyType& key, const ValueType& value) {
...
template <typename ValueType, typename KeyType ,typename HashFunc>
bool HashTable<ValueType, KeyType, HashFunc>::remove(const KeyType& key) {
    for (size_t i = 0; i < table.getCapacity(); ++i) {
        size_t index = hash(key, i);
        HashEntry& entry = table.get(index);
...
template <typename ValueType, typename KeyType ,typename HashFunc>
bool HashTable<ValueType, KeyType, HashFunc>::contains(const KeyType& key) const {
    for (size_t i = 0; i < table.getCapacity(); ++i) {
        size_t index = hash(key, i);
...
template <typename ValueType, typename KeyType ,typename HashFunc>
ValueType HashTable<ValueType, KeyType, HashFunc>::get(const KeyType& key) const {
    for (size_t i = 0; i < table.getCapacity(); ++i) {
        size_t index = hash(key, i);
        const HashEntry& entry = table.get(index);
        if (entry.isOccupied && entry.key == key) {
...
template <typename ValueType, typename KeyType ,typename HashFunc>
void HashTable<ValueType, KeyType, HashFunc>::printTable() const {
    for (size_t i = 0; i < table.getCapacity(); ++i) {
